...
Section ilist2.
  ...
  Lemma ilist2_invert {n} (As : Vector.t A n) (il : ilist2 As) :
    match As as As' return ilist2 As' -> Prop with
      | a :: As' => fun il => exists b il', il = icons2 _ b il'
      | [] => fun il => il = inil2
    end il.
  Proof.
  ...
  Lemma ilist2_invert' {n} (As : Vector.t A n) (il : ilist2 As) :
    match As as As' return ilist2 As' -> Type with
      | a :: As' => fun il => sigT (fun b => sigT (fun il' => il = icons2 _ b il'))
      | [] => fun il => il = inil2
    end il.
  Proof.
...
Section ilist2_imap.
  ...
  Fixpoint imap2 {n} (As : Vector.t A n)
    ...
    | [] => fun il => inil2
    | a :: As' => fun il => icons2 _ (@f a (ilist2_hd il)) (imap2 As' (ilist2_tl il))
    end.
    ...
    | Fin.F1 =>
      fun As =>
        Vector.caseS (fun n As => ilist2 As
                                  -> B (Vector.nth As (@Fin.F1 n))
                                  -> ilist2 As)
                     (fun h n t il new_b => icons2 _ new_b (ilist2_tl il) ) As
    | Fin.FS n' =>
      fun As =>
        Vector_caseS' Fin.t
                      (fun n As n' =>
                          ...
                          -> B (Vector.nth As (@Fin.FS n n'))
                          -> ilist2 As)
                     (fun h n t m il new_b => icons2 _ (ilist2_hd il)
...
Lemma imap_replace2_Index
  ...
  : forall (f : forall idx'', B idx'' -> B' idx'')
           ...
           (il : ilist2 Bound)
           b,
    imap2 _ B B' f Bound (replace_Index2 Bound il idx b) =
    replace_Index2 Bound (imap2 _ B B' f Bound il) idx (f _ b).
Proof.
  induction idx.
    ...
    | Fin.F1 =>
      fun As =>
        Vector.caseS (fun n As => ilist2 As
                                  ...
                                  -> ilist2 As)
                     (fun h n t il update_b => icons2 _ (update_b (ilist2_hd il)) (ilist2_tl il) ) As
    | Fin.FS n' =>
      fun As =>
        Vector_caseS' Fin.t
                      (fun n As n' =>
                          ...
                          -> ilist2 As)
                      (fun h n t m il update_b => icons2 _ (ilist2_hd il)
  ...
  Proof.
    ...
    - assert (f <> g) by congruence.
      ...
      match goal with
        |- ?P n0 As =>
        simpl; apply (@Vector.caseS _ P); intros;
        eapply (H _ (prim_snd _ _ il) update_b); eauto
      end.
  Qed.
  Set Printing All.
