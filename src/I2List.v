...
Section i2list.
  ...
  Fixpoint i2list {n}
    ...
    | [] => fun _ => unit
    | a :: l' => fun il => @prim_prod (C _ (ilist2_hd il))
                                      (i2list (ilist2_tl il))
    end.
  ...
  Definition i2list_hd {n}
    ...
    | a :: As' => fun Bs i2l => C _ (ilist2_hd Bs)
      | [] => fun _ _ => unit
    end Bs i2l :=
    ...
    | [] => fun _ _ => tt
    | a :: As' => fun Bs il => prim_fst _ _ il
    end Bs i2l.
  ...
  Definition i2list_tl {n}
    ...
    | [] => fun _ _ => tt
    | a :: As' => fun Bs il => prim_snd _ _ il
    end Bs i2l.
      ...
      fun As Bs i2l =>
        Vector.caseS (fun n As =>
                        forall (Bs : ilist2 As), i2list Bs
                                   -> C _ (ith2 Bs (@Fin.F1 n)))
                     (fun a' n' As' (Bs' : ilist2 (a' :: As')) i2l' => prim_fst _ _ i2l' ) As Bs i2l
    | Fin.FS n'' =>
      fun As Bs i2l =>
        Vector_caseS' Fin.t
                      (fun n' As i =>
                        forall (Bs'' : ilist2 As),
                          i2list Bs''
                          -> C _ (ith2 Bs'' (@Fin.FS n' i)))
                     (fun a' n' As' n'' (Bs' : ilist2 (a' :: As')) i2l' => i2th (prim_snd _ _ i2l') n'')
                     As n'' Bs i2l
    end As Bs i2l.
          ...
          fun As Bs i2l new_c =>
            Vector.caseS (fun n As =>
                            forall (Bs : ilist2 As), i2list C Bs
                                                     -> C _ (ith2 Bs (@Fin.F1 n))
                                                     -> i2list C Bs)
                         (fun a' n' As' (Bs' : ilist2 (a' :: As')) i2l' new_c => i2cons new_c (prim_snd _ _ i2l') ) As Bs i2l new_c
    | Fin.FS n'' =>
      fun As Bs i2l new_c =>
        Vector.caseS (fun n' As =>
                        forall (i : Fin.t n') (Bs'' : ilist2 As),
                          ...
                          -> C _ (ith2 Bs'' (@Fin.FS n' i))
                          -> i2list C Bs'')
                     (fun a' n' As' n'' (Bs' : ilist2 (a' :: As')) i2l' new_c => i2cons (prim_fst _ _ i2l') (replace2_Index2 (prim_snd _ _ i2l') n'' new_c))
                     As n'' Bs i2l new_c
        end As Bs i2l new_c.
